...
function Films(props) {
    ...
    const addFilm = (movie) => {
        // list.addNewFilmByRow(id, title, isFavorite = false, dateWatched, rating);
        props.setFilmList(library.addNewFilmByRow(
            movie.id,
            movie.title,
            movie.isFavorite,
            movie.dateWatched,
    ...
    const deleteFilm = (title) => {
        props.setFilmList([...library.getFilms().filter(f => f.title !== title)]);
      }
